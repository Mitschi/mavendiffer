 ...
 */
public class PomGreedyBottomUpMatcher extends Matcher {
    ...
    public PomGreedyBottomUpMatcher(ITree src, ITree dst, MappingStore store) {
        super(src, dst, store);
    }
    ...
    private void lastChanceMatch(ITree src, ITree dst) {
        ...
        if (cSrc.getSize() < SIZE_THRESHOLD || cDst.getSize() < SIZE_THRESHOLD) {
            Matcher m = new PomZsMatcher(cSrc, cDst, new MappingStore());
            m.match();
