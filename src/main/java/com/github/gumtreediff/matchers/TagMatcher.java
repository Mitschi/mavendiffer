...
public class TagMatcher extends Matcher {
	    ...
	    public TagMatcher(ITree src, ITree dst, MappingStore store) {
	        super(src, dst, store);
	    }

	    public void match() {
	    	List<String> allowedTags = Arrays.asList(new String[]{"dependency","plugin","repository"});
	    	List<String> firstLevelTags = Arrays.asList(new String[]{"parent","organization","licences"});
...
//	    	Set<String> allTags = new HashSet<String>(srcTreesOfTag.keySet());
//	    	allTags.addAll(dstTreesOfTag.keySet());
//	    	
...
//								mappings.link(t, bestMatch);
								...
								System.out.println(t.toShortString()+" "+t.getLabel()+" ("+getMavenTripleId(t)+")");
								System.out.print("  ");
								System.out.println(bestMatch.toShortString()+" "+bestMatch.getLabel()+" ("+getMavenTripleId(bestMatch)+")");
							}
						}
					}
				}
...
//			double sim=jaccardSimilarity(t, cand);
			double sim=0.0d;
			if("dependency".equals(t.getLabel()) || "plugin".equals(t.getLabel())) {
				String tTriple=getMavenTripleId(t);
				String candTriple=getMavenTripleId(cand);
				sim=levenshtein.compare(tTriple, candTriple);
			}
			if("repository".equals(t.getLabel())) {
				String tId = getId(t);
				String candId = getId(cand);
				sim=levenshtein.compare(tId, candId);
			}
			return sim;
			}
			else {
		...
		private String getId(ITree node) {
			ITree id = getChildByName(node, "id");
			
			return id.getChild(0).getLabel();
		}
